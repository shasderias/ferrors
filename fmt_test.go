...
func TestErrorFormatter(t *testing.T) {
	var (
		...
		framed    = &withFrameAndMore{
			frame: ferrors.Caller(0),
		}
		opaque = &wrapped{"outer",
			ferrors.Opaque(&wrapped{"mid",
				&wrapped{"inner", nil}})}
	)
	testCases := []struct {
		err    error
		...
		fmt: "%+v",
		want: "something:" +
			"\n    github.com/shasderias/ferrors_test.TestErrorFormatter" +
			"\n        .+/fmt_test.go:101" +
			"\n    something more",
		regexp: true,
	}, {
		...
		err:  simple,
		fmt:  "%T",
		want: "*ferrors_test.wrapped",
	}, {
		err:  simple,
		fmt:  "%🤪",
		want: "%!🤪(*ferrors_test.wrapped)",
		// For 1.13:
		//  want: "%!🤪(*ferrors_test.wrapped=&{simple <nil>})",
	}, {
...
var _ ferrors.Formatter = wrapped{}
...
func (e wrapped) Format(s fmt.State, verb rune) {
	ferrors.FormatError(&e, s, verb)
}

func (e wrapped) FormatError(p ferrors.Printer) (next error) {
	p.Print(e.msg)
	p.Detail()
...
var _ ferrors.Formatter = detailed{}

type detailed struct{}

func (e detailed) Error() string { panic("should have called FormatError") }

func (detailed) FormatError(p ferrors.Printer) (next error) {
...
func (e *withFrameAndMore) Format(s fmt.State, v rune) {
	ferrors.FormatError(e, s, v)
}

func (e *withFrameAndMore) FormatError(p ferrors.Printer) (next error) {
	p.Print("something")
	if p.Detail() {
		e.frame.Format(p)
...
func (e spurious) FormatError(p ferrors.Printer) (next error) {
	p.Print("spurious")
...
func (e *oneNewline) FormatError(p ferrors.Printer) (next error) {
	p.Print("1")
	p.Print("2")
	p.Print("3")
	p.Detail()
...
func (e *newlineAtEnd) FormatError(p ferrors.Printer) (next error) {
...
func (e adapted) FormatError(p ferrors.Printer) error {
	p.Print(e.msg)
	p.Detail()
	p.Print("detail")
	return e.err
...
// formatError is an error implementing Format instead of ferrors.Formatter.
...
func (e fmtTwiceErr) FormatError(p ferrors.Printer) (next error) {
...
var rePath = regexp.MustCompile(`( [^ ]*)ferrors.*test\.`)
...
func errToParts(err error) (a []string) {
	for err != nil {
		var p testPrinter
		if ferrors.Unwrap(err) != nil {
			p.str += "wraps:"
		}
		f, ok := err.(ferrors.Formatter)
		if !ok {
