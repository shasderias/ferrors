...
func TestNewEqual(t *testing.T) {
	...
	if ferrors.New("abc") == ferrors.New("abc") {
		t.Errorf(`New("abc") == New("abc")`)
	}
	if ferrors.New("abc") == ferrors.New("xyz") {
...
func TestNewDetail(t *testing.T) {
	got := fmt.Sprintf("%+v", ferrors.New("error"))
...
func ExampleNew() {
	err := ferrors.New("emit macho dwarf: elf header corrupted")
	if err != nil {
		fmt.Print(err)
	}
