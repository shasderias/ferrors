...
func TestIs(t *testing.T) {
	err1 := ferrors.New("1")
	erra := ferrors.Errorf("wrap 2: %w", err1)
	errb := ferrors.Errorf("wrap 3: %w", erra)
	erro := ferrors.Opaque(err1)
	errco := ferrors.Errorf("opaque: %w", erro)
	err3 := ferrors.New("3")
	...
	for _, tc := range testCases {
		t.Run("", func(t *testing.T) {
			if got := ferrors.Is(tc.err, tc.target); got != tc.match {
...
func TestAs(t *testing.T) {
	...
	testCases := []struct {
		...
		ferrors.Errorf("pittied the fool: %w", errorT{}),
		&errT,
		true,
	}, {
		errF,
		...
		ferrors.Errorf("path error: %w", errF),
		&timeout,
		true,
	}}
	for i, tc := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			match := ferrors.As(tc.err, tc.target)
			if match != tc.match {
				t.Fatalf("ferrors.As(%T, %T): got %v; want %v", tc.err, tc.target, match, tc.match)
			}
			if !match {
				return
			}
...
func TestUnwrap(t *testing.T) {
	...
	testCases := []struct {
		...
		{err1, nil},
		{erra, err1},
		{ferrors.Errorf("wrap 3: %w", erra), erra},

		{erro, nil},
		{ferrors.Errorf("opaque: %w", erro), erro},
	}
	for _, tc := range testCases {
		if got := ferrors.Unwrap(tc.err); got != tc.want {
...
func TestOpaque(t *testing.T) {
	got := fmt.Sprintf("%v", ferrors.Errorf("foo: %v", ferrors.Opaque(errorT{})))
	want := "foo: errorT"
	if got != want {
		t.Errorf("error without Format: got %v; want %v", got, want)
	}

	got = fmt.Sprintf("%v", ferrors.Errorf("foo: %v", ferrors.Opaque(errorD{})))
	want = "foo: errorD"
	if got != want {
...
func (errorD) FormatError(p ferrors.Printer) error {
	p.Print("errorD")
	p.Detail()
	p.Print("detail")
	return nil
